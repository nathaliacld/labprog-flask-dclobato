...
def add():
    ...
    categoria = db.session.execute(db.select(Categoria).order_by(Categoria.nome)).scalars()
    form.categoria.choices = [(str(i.id), i.nome) for i in categoria]
    ...
    return render_template('produto/add_edit.jinja2',
                           form=form,
